...
function buildMaterialLine(item, quantity) {
  ...
  const meta = {
    name: item.name || "", // denominación normalizada, igual a catalogos/normalizacion_sku.csv — la usa el export a vigia.py
...
export function generateQuotePdf(quote, targetWindow = null) {
  ...
  const adminLabel = `Gastos administrativos (${Math.round((quote.adminOverheadRate ?? 0) * 10000) / 100}%)`;
  const contingencyLabel = `Contingencia técnica (${Math.round((quote.technicalContingencyRate ?? 0) * 10000) / 100}%)`;
      ...
      <body>
        <main class="sheet">
          ...
          <section class="totals">
            ...
            ${quote.adminAmount ? `<div><span>${adminLabel}</span><strong>${money(quote.adminAmount)}</strong></div>` : ""}
            ${quote.contingencyAmount ? `<div><span>${contingencyLabel}</span><strong>${money(quote.contingencyAmount)}</strong></div>` : ""}
...
const Cotizador = forwardRef(function Cotizador(
  {
    companies, materials, laborRates, quoteParameters, step, setStep, lightboxImage, setLightboxImage,
    initialQuote = null, activeQuoteId = null, activeQuoteNumber = null,
    onQuoteChange = () => null, onCreateCompany = () => {}, onDone = () => {}, onSavingStatusChange = () => {},
  },
  ref
) {
  ...
  const adminOverheadRate = Number(quoteParameters.adminOverhead || 0);
  const technicalContingencyRate = Number(quoteParameters.technicalContingency || 0);
  const adminAmount = Math.round(subtotal * adminOverheadRate);
  const contingencyAmount = Math.round(subtotal * technicalContingencyRate);
  const taxableBase = subtotal + adminAmount + contingencyAmount;
  const ivaRate = Number(quoteParameters.iva || 0);
  const tax = Math.round(taxableBase * ivaRate);
  const total = roundToMultiple(taxableBase + tax, quoteParameters.roundTo);
  ...
  const initialStateRef = useRef({ lineItems, clientDetails, jobTitle, validUntil });

  useEffect(() => {
    // Sin cambios del usuario no se guarda: si no, abrir el cotizador ya consumia
    // un numero de presupuesto (la empresa viene precargada). Identidad, no deep-equal.
    const init = initialStateRef.current;
    if (lineItems === init.lineItems && clientDetails === init.clientDetails && jobTitle === init.jobTitle && validUntil === init.validUntil) return;
    ...
    const timer = setTimeout(async () => {
      ...
      const saved = await currentOnChange(currentId, draft);
  ...
  async function saveQuote({ openPdf = false, pdfWindow = null } = {}) {
    ...
    try {
      ...
      if (clientMode === "new" && companyName && !companies.some((company) => normalizeKey(company.name) === normalizeKey(companyName))) {
        ...
        await onCreateCompany(record);
      }

      const draft = buildQuote(quoteNumber);
      const saved = await onQuoteChange(activeQuoteId, draft);
  ...
  if (step === "resumen") {
    return (
      <ResumenStep
        ...
        subtotal={subtotal} adminAmount={adminAmount} adminOverheadRate={adminOverheadRate}
        contingencyAmount={contingencyAmount} technicalContingencyRate={technicalContingencyRate}
        tax={tax} total={total} ivaRate={ivaRate} money={money}
