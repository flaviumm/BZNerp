...
import { Panel, Button, TextInput, Icon, Badge } from "../ui.jsx";
...
export default function ResumenStep({
  subtotalMaterials, subtotalLabor, subtotalTravel, subtotalOther, subtotal,
  adminAmount, adminOverheadRate, contingencyAmount, technicalContingencyRate,
  tax, total, ivaRate, money,
  lineItems, updateLine, removeLine, addLine, quoteLineTotal, generatedQuote, saving, handleGeneratePdf, onEdit, validUntil,
}) {
  const ivaPercentLabel = Math.round((ivaRate || 0) * 100 * 100) / 100;
  const adminPercentLabel = Math.round((adminOverheadRate || 0) * 100 * 100) / 100;
  const contingencyPercentLabel = Math.round((technicalContingencyRate || 0) * 100 * 100) / 100;
  return (
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
        ...
        <div className="sticky top-20 self-start rounded-lg border border-outline bg-inverse-surface p-6 text-white">
          ...
          <div className="mb-4 space-y-3 text-sm">
            ...
            {adminAmount > 0 && <div className="flex justify-between text-white/80"><span>Gastos administrativos ({adminPercentLabel}%)</span><span className="font-mono">{money(adminAmount)}</span></div>}
            {contingencyAmount > 0 && <div className="flex justify-between text-white/80"><span>Contingencia técnica ({contingencyPercentLabel}%)</span><span className="font-mono">{money(contingencyAmount)}</span></div>}
            <div className="flex justify-between text-white/80"><span>IVA ({ivaPercentLabel}%)</span><span className="font-mono">{money(tax)}</span></div>
          </div>
